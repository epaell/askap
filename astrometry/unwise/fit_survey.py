...
# Extracts per-beam catalogues and merges them into a single field catalogue containing all survey sources.
def load_survey(survey, field_name):
    field_cat = None
    print(field_name)
    if os.path.exists(f"{survey}/{survey}_{field_name}.fits") == True:
        field_cat = Table.read(f"{survey}/{survey}_{field_name}.fits", format="fits")
...
def get_offset_space(cata, survey_table, window, plot=True, radius_deg: float=5.):
    survey_sky = SkyCoord(survey_table["RAJ2000"], survey_table["DEJ2000"], unit=(u.deg, u.deg))
    ...
    matches = match_coordinates_sky(shifted_sky, survey_sky, nthneighbor=1, storekdtree=True)
...
if (catas[0].cat_type in ["selavy"])==True:
    fout = open(f"SB{sbid}.{field_name}.{survey}.selavy.shifts.csv", "wt")
else:
    fout = open(f"SB{sbid}.{field_name}.{survey}.shifts.csv", "wt")
...
survey_field_cat = load_survey(survey, field_name)
...
#        print(window)
        offset_results = get_offset_space(catas[beam], survey_field_cat, window=window)
    ...
    fout.write("%d,%f,%f\n" %(beam, min_ra, min_dec))
os.system(f"montage SB{sbid}*beam*.png -geometry +6+6 SB{sbid}.{field_name}.{survey}.offset_grid.png")
